...
def profile_wind(scenarij):
    ...
    datoteka3 = "wind_normalized_au.xlsx"
    podatki3 = pd.read_excel(PATH +"\\"+ datoteka3)
    podatki3.set_index(podatki3.columns[0], inplace=True)


    time_series_per_year_wind_list = []
    time_series_per_year_wind_au_list = []
    for year in range(zacetno_leto, koncno_leto + 1):
        ...
        df_wind_au = pd.DataFrame(index=hourly_index)

        # 3. Data from excel
        anual_total = podatki.loc["wind",year] # je profil že normiran na MWh
        profil_norm = podatki2
        profil_norm_au = podatki3
        ...
        if isleap(year):
            ...
            df_wind_au["profil"] = profil_norm_au["Profil_norm"].values*anual_total
        else:
            profil_norm = profil_norm[~((profil_norm.index.month == 2) & (profil_norm.index.day == 29))]
            profil_norm_au = profil_norm_au[~((profil_norm_au.index.month == 2) & (profil_norm_au.index.day == 29))]
        ...
        df_wind_au.index.name = "Časovna značka"

        # 8. Save to dictionary
        #time_series_per_year_osnovni[year] = df
        time_series_per_year_wind_list.append(df_wind)
        time_series_per_year_wind_au_list.append(df_wind_au)

    time_series_per_year_wind = pd.concat(time_series_per_year_wind_list)
    time_series_per_year_wind_au = pd.concat(time_series_per_year_wind_au_list)

    return time_series_per_year_wind, time_series_per_year_wind_au
